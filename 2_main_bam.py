...
#data = ['con_1']
#data = ['pru_1']
#data = ['con_1','tur_1','tur_2','cmi_1','cmi_2','cmi_3']
...
# =====================================================================
    ...
    for dat in data: 
        ...
        with open(os.path.join(tmppathres,'bam_'+dat)+'.pkl','wb')      as f: pickle.dump(bam, f)
        with open(os.path.join(tmppathres,'bam_dic_'+dat)+'.pkl','wb')  as f: pickle.dump(bam_dic, f)
    ...
    print(f'\n        Time (min) --> fase2: {(time.time() - start_total)/60}')
    #print(f'Total Time: {time.time() - start_total}')
    print('\b')
    print(dat+':')
    print('------')
    print('\bSum of row/colum differences by year:')
    print('\b')
    print(sum_difs_df[dat].sum())
    print('\b')
    finish_bam = time.time()
    local_time = time.ctime(finish_bam)
